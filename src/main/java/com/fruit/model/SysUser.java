...
import com.fruit.model.base.BaseSysUser;
...
public class SysUser extends BaseSysUser<SysUser> {
    ...
    public InvokeResult save(Integer id, String username, String realName, String password, String des, String phone, String email, String wxid) {
        if (null != id) {
            SysUser sysUser = this.findById(id);
            sysUser.set("realName", realName).set("des", des).set("phone", phone).set("email", email).set("wxid", wxid);
            if (!StringUtils.isEmpty(password)) {
                sysUser.set("pwd", MyDigestUtils.shaDigestForPasswrod(password));
            }
            sysUser.update();
        } else {
            if (this.hasExist(username)) {
                ...
                sysUser.set("name", username).set("pwd", MyDigestUtils.shaDigestForPasswrod(password)).set("createdate", new Date()).set("realName", realName).set("des", des).set("phone", phone).set("email", email).set("wxid", wxid).save();
            }
        }
        return InvokeResult.success();
    }
